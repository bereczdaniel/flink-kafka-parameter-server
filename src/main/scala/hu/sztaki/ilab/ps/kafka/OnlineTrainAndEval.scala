  ...
  def parameterParseAndRun(args: Array[String]): Unit =
    Utils.getParameters(args) match {
      case Some(parameters) =>
        ...
        val logBackend = parameters.get("logBackend", "couchbase")
        ...
        run(algorithm, psImplType, parallelism, inputFile, outputFile, snapshotLength, doEvalAndWrite, withDataStreamLogger, logBackend, K, parameters)
      case None =>
    }
  ...
  def run(algorithm: String,
    ...
    val psOutput =  if (withDataStreamLogger) {
      JobLogger.doWithLogging[EvaluationRequest, Recommendation](createInput(env, inputFile),
        DbWriterFactory.createDbWriter(logBackend, parameters), env, psImplType,
...
//        EvaluationRequest(fields(2).toInt, fields(3).toInt, fields(0).toLong, 1.0, fields(1).toLong - 1390209861L)
  ...
  //TODO rename : not run but build up the job graph - adds the result collection
  //TODO runPS and createPS can be merged to buildPS
